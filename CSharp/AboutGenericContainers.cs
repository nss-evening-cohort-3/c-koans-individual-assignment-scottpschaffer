...
namespace DotNetKoans.CSharp
{
	public class AboutGenericContainers : Koan
	{
		...
		public void BasicStackPushingAndPopping()
		{
			...
			stack.Push("last");
            object[] x = new object[] { "last", 2, 1 };
            object[] y = new object[] { 2, 1 };
            Assert.Equal(x, stack.ToArray());
			var poppedValue = stack.Pop();
			Assert.Equal("last", poppedValue);
			Assert.Equal(y, stack.ToArray());
		}
		...
		public void Shifting()
		{
            ...
            string[] x = new string[] { "Say", "Hello", "World" };
			list.AddFirst("Say");
			Assert.Equal(x, list.ToArray());

			list.RemoveLast();
            string[] y = new string[] { "Say", "Hello" };
            Assert.Equal(y, list.ToArray());

			list.RemoveFirst();
            string[] z = new string[] { "Hello" };
            Assert.Equal(z, list.ToArray());

			list.AddAfter(list.Find("Hello"), "World");
            string[] a = new string[] { "Hello", "World" };
            Assert.Equal(a, list.ToArray());
		}
		[Koan(21)]
